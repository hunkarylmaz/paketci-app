// Paketçi Extension - Migros Detector

const MigrosDetector = {
  platform: 'migros',
  
  isMatch() {
    return window.location.hostname.includes('migros.com.tr');
  },

  detectOrders() {
    const orders = [];
    const selectors = CONSTANTS.SELECTORS.migros;
    ...
    containers.forEach(container => {
      try {
        ...
        console.error('[Paketçi] Migros parse error:', error);
      }
    });
...
window.MigrosDetector = MigrosDetector;
