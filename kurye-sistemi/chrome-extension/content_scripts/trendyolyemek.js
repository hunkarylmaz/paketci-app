...
(function() {
  ...
  function init() {
    console.log('Paketçiniz: Trendyol Yemek content script loaded');
    observeOrders();
    checkForOrders();
  }
  ...
  function extractOrderData(element) {
    try {
      ...
      console.error('Paketçiniz: Error extracting Trendyol order', error);
      return null;
    }
  }
  ...
  function extractItems(element) {
    const items = [];
    const itemElements = element.querySelectorAll('.order-item, [class*="item"]');
    
    itemElements.forEach(itemEl => {
      const name = findText(itemEl, ['.product-name', '.item-name']);
      const quantity = findNumber(itemEl, ['.quantity']) || 1;
      const price = findNumber(itemEl, ['.price']) || 0;
