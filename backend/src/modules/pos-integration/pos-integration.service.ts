...
export class POSIntegration {
  ...
  @Column({ type: 'enum', enum: ['ADISYO', 'SEPETTAKIP', 'SEFIM', 'OTHER'] })
  posType: string;
...
export class CallerIDDevice {
  ...
  @Column({ type: 'enum', enum: ['USB', 'SERIAL', 'NETWORK', 'SOFTWARE'] })
  deviceType: string;
...
export class Printer {
  ...
  @Column({ type: 'enum', enum: ['THERMAL', 'LABEL', 'IMPACT', 'MOBILE'] })
  printerType: string;

  @Column({ type: 'enum', enum: ['BLUETOOTH', 'USB', 'WIFI', 'ETHERNET', 'SERIAL'] })
